...
export default function StoneCreek() {
  const images = useMemo(() => {
    const total = 12;
    const paths: string[] = [];
    for (let n = 1; n <= total; n++) {
      const two = String(n).padStart(2, '0');
      paths.push(`/project-stone-creek/205 Stone Creek Ct High Res_${two}.jpg`);
    }
    return paths;
  }, []);

  const encodedImages = useMemo(() => images.map((p) => encodeURI(p)), [images]);
  const videoSrc = useMemo(() => encodeURI('/project-stone-creek/stone-creek-video.mp4'), []);
        ...
        <div className="w-[80%] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-14 items-start">
          <div className="lg:col-span-7">
            <h2 className="text-3xl md:text-4xl font-normal text-text-strong mb-6" style={{ fontFamily: "'PP Editorial Old', serif" }}>
              A Luxury Custom Home Nestled in a Private Estate Enclave
            </h2>
            <div className="space-y-5 text-text leading-relaxed text-lg">
              <p>
                Project Stone Creek is a luxury custom home nestled in a private enclave of five estates in Woodstock, Georgia.
                Featuring nearly two acres of serene landscape, it blends timeless design with modern comfort — from a chef's kitchen
                with custom white oak cabinetry and Cambria countertops to a terrace-level theater and wellness room.
              </p>
              <p>
          ...
          <div className="lg:col-span-5">
            <div>
              ...
              <ul className="space-y-2 text-text">
                <li>• Chef's kitchen with custom white oak cabinetry, Cambria countertops, and a 48" Viking range</li>
                <li>• Terrace-level entertainment space with golf simulator/theater and wellness room</li>
                <li>• Expansive rear porch overlooking rolling pastures and scenic Woodstock views</li>
              </ul>
            </div>
          </div>
        </div>
        ...
        <div className="w-[80%] mx-auto">
          <div className="relative overflow-hidden">
            <AspectRatio ratio={16/7} className="min-h-[320px] sm:min-h-[380px] md:min-h-[460px]">
              <img src={encodedImages[2] ?? encodedImages[0]} alt="Stone Creek interior" className="w-full h-full object-cover" />
            ...
            <div className="absolute inset-0 p-6 md:p-10 flex flex-col justify-end">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                  { phase: 'Design', when: 'Mar – May 2024' },
                  { phase: 'Site & Foundation', when: 'Jun – Jul 2024' },
                  { phase: 'Framing & Systems', when: 'Aug – Nov 2024' },
                  { phase: 'Finishes & Completion', when: 'Dec 2024 – Feb 2025' },
                ].map((t, i) => (
            ...
            <Dialog key={idx}>
              <DialogTrigger asChild>
                <button className="break-inside-avoid mb-4 block focus:outline-none">
                  <img src={src} alt={`Stone Creek ${idx + 1}`} className="w-full h-auto" />
                </button>
              </DialogTrigger>
              <DialogContent className="max-w-5xl p-0 bg-transparent border-none shadow-none sm:rounded-none [&>button]:absolute [&>button]:right-4 [&>button]:top-4 [&>button]:bg-black/50 [&>button]:text-white [&>button]:rounded-full [&>button]:p-2 [&>button]:hover:bg-black/70">
                <img src={src} alt={`Stone Creek ${idx + 1} large`} className="w-full h-auto max-h-[85vh] object-contain" />
              </DialogContent>
            </Dialog>
          ))}
        </div>
